...
-- Function to create a match record when bracket_match becomes ready
CREATE OR REPLACE FUNCTION public.create_match_for_bracket(
...
BEGIN
  -- Get bracket match details
  SELECT bm.*, bg.number as group_number, br.number as round_number
  INTO v_bracket_match
  FROM public.bracket_match bm
  JOIN public.bracket_round br ON br.id = bm.round_id
  JOIN public.bracket_group bg ON bg.id = bm.group_id
  WHERE bm.id = p_bracket_match_id;

  IF v_bracket_match IS NULL THEN
    RAISE EXCEPTION 'Bracket match not found: %', p_bracket_match_id;
  END IF;

  -- Get team IDs from participants
  SELECT bp.team_id INTO v_team_one_id
  FROM public.bracket_participant bp
  WHERE bp.id = (v_bracket_match.opponent1->>'id')::INTEGER;

  SELECT bp.team_id INTO v_team_two_id
  FROM public.bracket_participant bp
  WHERE bp.id = (v_bracket_match.opponent2->>'id')::INTEGER;

  -- Determine bracket side and round name
  IF v_bracket_match.group_number = 1 THEN
    v_bracket_side := 'upper';
    v_round_name := 'Winners Round ' || v_bracket_match.round_number;
  ELSIF v_bracket_match.group_number = 2 THEN
    v_bracket_side := 'lower';
    v_round_name := 'Losers Round ' || v_bracket_match.round_number;
  ELSE
    v_bracket_side := 'final';
    v_round_name := 'Grand Final';
  END IF;
  ...
  IF v_match_id IS NOT NULL THEN
    ...
    UPDATE public.match
    SET team_one_id = v_team_one_id,
        team_two_id = v_team_two_id
    WHERE id = v_match_id;
    RETURN v_match_id;
  END IF;
...
GRANT EXECUTE ON FUNCTION public.create_match_for_bracket TO authenticated;
